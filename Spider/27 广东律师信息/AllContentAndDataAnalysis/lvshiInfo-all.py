...
post 请求字段
pagerOrgParam.tbOrgLsglJgjbxxVo.fwxzqh:
...
def getLvsuoInfo(oid,diqu1,page1): #page1是传进来的值 便于检索所有的页面是否抓取到完整的信息
    ...
    opiCheckyear = data['tbOrgLsglLsjgzyxxVoList'][0]['opiCheckyear']  # 最新考核年份
    ...
    tplt = "{0:^13}\t{1:^4}\t{2:^4}\t{3:^8}\t{4:^2}\t{5:^10}\t{6:^3}\t{7:^3}\t{8:^19}\t{9:^22}\t{10:^6}\t{11:^20}\t{12:^2}\t{13:^10}"
    print(tplt.format(obiOrgname,obiManagerlevelDesc,opiOorganformDesc,obiOrgtypeDesc,obiOfficeused,obiFounddate,opiLeadername,opiCheckyear,creditcode ,obiEmail,obiPostal,obiAddress,proplenum,diqu1))
    sql_2 = """
        INSERT IGNORE INTO gdlawyer (obiOrgname,obiManagerlevelDesc,opiOorganformDesc,obiOrgtypeDesc,obiOfficeused,obiFounddate,opiLeadername,opiCheckyear,creditcode ,obiEmail,obiPostal,obiAddress,proplenum,diqu1)VALUES('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}'  )
                        """ \
        .format(
        pymysql.escape_string(obiOrgname),
        pymysql.escape_string(obiManagerlevelDesc),
        pymysql.escape_string(opiOorganformDesc),
        pymysql.escape_string(obiOrgtypeDesc),
        pymysql.escape_string(obiOfficeused),
        pymysql.escape_string(obiFounddate),
        pymysql.escape_string(opiLeadername),
        pymysql.escape_string(opiCheckyear),
        pymysql.escape_string(creditcode),
        pymysql.escape_string(obiEmail),
        pymysql.escape_string(obiPostal),
...
if __name__=='__main__':
    for j in range(1, 343):
        # 将字符串转为列表便于美化输出
        navigation = "律所名 主管机关 组织形式 机构类型 设立资产（单位万） 成立时间 负责人 最新考核年份 统一社会信用码 邮箱 邮政编码 地址 人数 地区"
        navigationList = navigation.split(' ')
        ...
        # 打印对其方式
        tplt = "{0:^18}\t{1:^4}\t{2:^4}\t{3:^3}\t{4:^2}\t{5:^10}\t{6:^3}\t{7:^3}\t{8:^19}\t{9:^22}\t{10:^6}\t{11:^20}\t{12:^2}\t{13:^10}"
        print(tplt.format(navigationList[0],navigationList[1],navigationList[2],navigationList[3],navigationList[4],navigationList[5],navigationList[6],navigationList[7],navigationList[8],navigationList[9],navigationList[10],navigationList[11],navigationList[12],navigationList[13]))
        # print("律所名  主管机关 组织形式  机构类型  设立资产（单位万） 成立时间  负责人 最新考核年份 统一社会信用码 邮箱 邮政编码 地址 人数 地区 ")
