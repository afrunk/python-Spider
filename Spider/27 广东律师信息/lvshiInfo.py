...
def getLvsuoInfo(oid,diqu):
    ...
    data = obiOrgname+','+opiOorganformDesc+','+obiOfficeused+','+obiFounddate+','+creditcode+','+obiAddress+','+proplenum+','+diqu
    print(obiOrgname,opiOorganformDesc,obiOfficeused,obiFounddate,creditcode,obiAddress,proplenum,diqu)
    with open("广州.csv", 'a+', newline='') as f:  # 写入到本地csv中 a+会自动创建文件 newline解决中间有空行的问题
        # write = csv.writer(f)
        f.writelines(data)
        f.write('\n')
